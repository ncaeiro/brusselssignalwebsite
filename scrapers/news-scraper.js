...
// Function to extract news article details from detail page
async function fetchNewsArticleDetails(url) {
  try {
    console.log(`  → Fetching news article details from ${url}`);
    ...
    // Fallback to card author if still not found (common in news articles)
    if (!author) {
...
// Function to extract news article URLs from listing page
function extractNewsArticleUrls(html) {
  const urls = [];
...
async function scrapeNewsArticles(startPage = 1, endPage = 10) {
  const baseUrl = 'https://brusselssignal.eu/category/news/';
  const allNewsArticles = [];

  console.log('Starting news article scrape from Brussels Signal...\n');
  console.log(`Fetching news articles from pages ${startPage} to ${endPage}.\n`);
  ...
  for (let page = startPage; page <= endPage; page++) {
    try {
      ...
      const articleUrls = extractNewsArticleUrls(html);

      console.log(`✓ Found ${articleUrls.length} news articles on page ${page}`);
      ...
      for (let i = 0; i < articleUrls.length; i++) {
        ...
        const details = await fetchNewsArticleDetails(url);
        ...
        allNewsArticles.push(newsArticle);
      ...
      console.log(`✓ Completed page ${page}. Total articles so far: ${allNewsArticles.length}`);
  ...
  console.log(`\n\n✓ Total news articles scraped: ${allNewsArticles.length}`);
  console.log(`✓ Articles with titles: ${allNewsArticles.filter(a => a.headerTitle).length}`);
  console.log(`✓ Articles with authors: ${allNewsArticles.filter(a => a.author).length}`);
  console.log(`✓ Articles with text content: ${allNewsArticles.filter(a => a.articleText).length}`);
  console.log(`✓ Articles with images: ${allNewsArticles.filter(a => a.articleImage).length}`);
  console.log(`✓ Articles with image captions: ${allNewsArticles.filter(a => a.imageCaption).length}`);
  console.log(`✓ Articles with tags: ${allNewsArticles.filter(a => a.tags.length > 0).length}`);

  // Save to JSON file
  await fs.writeFile('output/news-articles.json', JSON.stringify(allNewsArticles, null, 2));
  console.log('\n✓ Data saved to output/news-articles.json');

  return allNewsArticles;
}

// Run the scraper for all pages (1 to 10)
scrapeNewsArticles(1, 10).catch(console.error);
